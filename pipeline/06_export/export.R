...
aggregate_park_stats <- function(rows, max_expected, cell_taxa_lookup = list(), patch_metrics = NULL) {
  ...
  list(
    ...
    treeCoverNorm = round(replace_na(finite_mean(rows$tree_cover_norm), replace_na(finite_first(patch_metrics$tree_cover_norm), 0.5)), 4),
    meanLstNorm = round(replace_na(finite_mean(rows$lst_norm), replace_na(finite_first(patch_metrics$mean_lst_norm), 0.5)), 4),
