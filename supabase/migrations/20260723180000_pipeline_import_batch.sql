-- Batched pipeline import for large cell_attributes exports.
-- R sends each export chunk (~24k cells) separately instead of one 200MB+ JSON payload.
...
begin
  ...
  -- Upsert dataset metadata only. Cell/green-space rows are upserted per batch;
  -- prior snapshot rows for this dataset_id are purged in import_pipeline_dataset_prepare.
...
create or replace function public.import_pipeline_dataset_finalize(
  target_city_id text,
  target_dataset_id text,
  target_generated_at timestamptz,
...
begin
  ...
  if expected_cell_count is not null and imported_cell_count <> expected_cell_count then
    raise exception 'Imported cell row count mismatch for %.%: expected %, found %.',
      target_city_id, target_dataset_id, expected_cell_count, imported_cell_count
      using errcode = 'check_violation';
  end if;
  ...
  return jsonb_build_object(
    ...
    'cellFeatureCount', expected_cell_count,
...
grant execute on function public.import_pipeline_dataset_prepare(text, text, timestamptz, text, text, text) to authenticated;
grant execute on function public.import_pipeline_dataset_cells_batch(text, text, timestamptz, jsonb) to authenticated;
grant execute on function public.import_pipeline_dataset_finalize(text, text, timestamptz, integer, jsonb, boolean) to authenticated;
