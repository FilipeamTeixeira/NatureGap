  ...
  const features = base.features.map((f) => {
    ...
    return {
      ...
      properties: {
        ...
        expectedRichness: cell.expectedRichness,
        ecologicalResidual: cell.ecologicalResidual,
        interventionRank: cell.interventions[0]?.id ? Number(cell.interventions[0].id.match(/rank-(\d+)/)?.[1] ?? 50) : 50,
