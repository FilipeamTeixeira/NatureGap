...
| `parks.geojson` | `pipeline/06_export/export.R` | Storage frontend | WGS84 polygons, stable `id`, no ecological metrics |
| `cell_attributes.geojson` | `pipeline/06_export/export.R` | Storage/archive + optional PostgreSQL import | Full ecological cell outputs with `city_id`, `dataset_id`, `generated_at` |
| `cell-details.manifest.json` + `cell-details/*` | `pipeline/06_export/export.R` | Frontend cell detail | Sharded per-cell detail keyed by `cell_id` |
